...
async function run() {
  try {
    ...
    app.put("/updateitem/:id", async (req, res) => {
      const id = req.params.id;
      const query = { _id: new ObjectId(id) };
      const getitem = await menuCollection.findOne(query);
      const item = req.body;
      const option = { upsert: true };
      const updatedItem = {
        $set: {
          name: item.name || getitem.name,
          recipe: item.recipe || getitem.recipe,
          image: item.image || getitem.image,
          category: item.category || getitem.category,
          price: item.price || getitem.price,
        },
      };
